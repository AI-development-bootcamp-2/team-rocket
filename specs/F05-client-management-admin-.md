...
- [ ] POST /clients — create (Admin only): name, contact_info (optional). Default is_active=true. **If name already exists (case-insensitive): return 201 with `{ data: {...}, warning: 'A client with this name already exists' }` — warn but allow (per v3.2 section 6).**
...
- [ ] Create/edit via modal form (ClientForm.jsx)
